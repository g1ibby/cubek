...
use cubek_reduce::{ReduceStrategy, launch::VectorizationStrategy, routines::{BlueprintStrategy, plane::PlaneStrategy}};
...
pub fn test_plane_reduction_simple() {
    let strategy = ReduceStrategy {
        vectorization: VectorizationStrategy{ parallel_output_vectorization: true },
        routine: RoutineStrategy::Plane(BlueprintStrategy::Inferred(PlaneStrategy{ independent:  true })),
    };
    let test_case = TestCase::new::<TestDType>(
        Shape::new([16]),
        Strides::new(&[1]),
        Some(0),
        strategy,
